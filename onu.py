...
class ONU(object):
    def __init__(self,oid,env,monitoring,wavelengths,distance,odn,fog_odn=None,fog_distance=3):
        ...
        self.bandwidth = 10000000000 #25Gbs
        self.env = env
    ...
    def ONU_ReceiverFromOLT(self):
        while True:
            ...
            grant_dict = {}
            print("ONU {} comecou com - buffer {} at {}".format(self.oid,self.buffer_size,self.env.now))
    ...
    def grant_processing(self,grant_list):
        ...
        for grant in grant_list:
            try:
                ...
                print self.env.now
                print( "ONU {} - tempo negativo ({})".format(self.oid, e))
                print grant['start']
                print("next_grant timeout {}".format(float(grant['start'] - self.env.now)))
                pass
            ...
            self.grant_usage = 0
        print("ONU {} terminou com - buffer {} at {}".format(self.oid,self.buffer_size,self.env.now))

    def SendUpDataToOLT(self,wavelength):
        pkt = yield self.buffer.get()
        self.buffer_size -= pkt.size
        print("pkt size {}".format(pkt.size))
        bits = pkt.size * 8
