...
for mhz in MHZs:
	...
	r2_UL= Fs * nAnt * n_Sector * nIQ
	MHz_splits[mhz][2]['bw'] = r2_UL

	gops_2 = int((int_cpri_option*nAnt*n_Sector*nIQ*nIQ)/100)
	...
	r6_UL = a6_UP * LayersUL + ( FAPI_UL * n_UE)
	gops_6 = int(a6_UP*LayersUL)
	MHz_splits[mhz][6]['bw'] = r6_UL
	MHz_splits[mhz][6]['gops'] = gops_6
	print "Split6 : %.3f Mbps	GOPS:%d |" % (r6_UL,gops_6)

	# SPLIT 7 -> SPLIT MAC - SCF
	a7_UP = (IP_TTI_UL* (IPpkt + HdrPDCP + HdrRLC + HdrMAC) * nTBS_UL_TTI)/ 125
	r7_UL = a7_UP * LayersUL + ( Sched * n_UE)
	gops_7 = int(a7_UP*LayersUL)
	MHz_splits[mhz][7]['bw'] = r7_UL
	MHz_splits[mhz][7]['gops'] = gops_7
	print "Split7 SPLIT MAC : %.3f Mbps	GOPS:%d |" % (r7_UL,gops_7)

	# SPLIT 8 -> SPLIT RLC-MAC - SCF
	a8_UP = (IP_TTI_UL* (IPpkt + HdrPDCP + HdrRLC) * nTBS_UL_TTI)/ 125
	r8_UL = a8_UP * LayersUL
	gops_8 = int(a8_UP*LayersUL)
	MHz_splits[mhz][8]['bw'] = r8_UL
	MHz_splits[mhz][6]['gops'] = gops_8
	print "Split8 SPLIT RLC-MAC : %.3f Mbps	GOPS:%d |" % (r8_UL,gops_8)

	# SPLIT 8 -> SPLIT PDCP-RLC - SCF
	a9_UP = (IP_TTI_UL* (IPpkt + HdrPDCP) * nTBS_UL_TTI)/ 125
	r9_UL = a9_UP * LayersUL
	gops_9 = int(a9_UP*LayersUL)
	MHz_splits[mhz][9]['bw'] = r9_UL
	MHz_splits[mhz][9]['gops'] = gops_9
	print "Split9 SPLIT PDCP-RLC : %.3f Mbps	GOPS:%d |" % (r9_UL,gops_9)

	# SPLIT 10 -> SPLIT RRC-PDCP - SCF
	a10_UP = (IP_TTI_UL * IPpkt * nTBS_UL_TTI) / 125
	r10_UL = a10_UP * LayersUL
	...
	MHz_splits[mhz][10]['bw'] = r10_UL
	MHz_splits[mhz][10]['gops'] = gops_10
	#########
	...
	for split in range(1,11):
		...
		metro_gops=0
		if split < 10:
			for metro_split in range(split,10):
...
########### PRINTING BW MATRIX WITH PANDAS
...
# 		print "%f" % (UL_splits[cpri][split]['bw'])
# 		#pass
